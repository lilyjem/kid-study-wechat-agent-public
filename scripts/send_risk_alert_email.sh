...
SMTP_SEND_SCRIPT="${SMTP_SEND_SCRIPT:-}"
ALERT_EMAIL_TO="${ALERT_EMAIL_TO:-}"
ALERT_SUBJECT_PREFIX="${ALERT_SUBJECT_PREFIX:-[儿童风险提醒]}"

if [[ -z "$SMTP_SEND_SCRIPT" ]]; then
  echo "Missing SMTP_SEND_SCRIPT. Example: export SMTP_SEND_SCRIPT=/path/to/send_email.py" >&2
  exit 3
fi
...
if [[ ! -f "$SMTP_SEND_SCRIPT" ]]; then
  echo "SMTP_SEND_SCRIPT does not exist: $SMTP_SEND_SCRIPT" >&2
  exit 5
fi
